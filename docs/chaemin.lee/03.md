...
- 요약하자면 Sender Thread는 전송할 준비가 되어있다면 Record Accumulator에 존재하는 배치를 가져간다. 여기서 가져가는 배치는 1개 또는 여러 개가 될 수 있고, batch.size 만큼 배치가 차지 않아도 가져가게 된다. 이런 경우에서 나의 서비스의 Sender Thread는 배치가 꽉 차지 않아도 가져간다면, linger.ms를 늘려서 대기시간을 길게 잡아서 배치가 어느정도 차게 한 다음 가져가게 할 수 있다. 또한 Sender Thread가 더 많은 배치를 한번에 가져가길 원한다면 linger.ms를 더 늘릴수도 있다.
...
- **브로커 서버의 응답없이** Producer의 sender thread가 **한번에 보낼 수 있는 메시지 배치의 개수의 Default 값은 5**이다.
