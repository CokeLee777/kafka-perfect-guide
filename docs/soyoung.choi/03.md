...
- 메시지 a가 **ㅁ모든 replicator에 완벽하게 복사되었는지의 여부는 확인하지 않고 메시지 b를 전송**
...
3-3 record accumulator2.png

3-4 record accumulator3.png
...
**프로듀서 id와 메시지 squence 를 header 에 저장하여 전송한다.**

그리고 브로커에 key = pid , value = message sequence 형태로 저장한다.

그리고 다음 메시지를 받을 때, pid 를 key 로 squecne 를 찾게 되고,  
  
이미 있는 suqeunce 메시지를 보냈을 경우, **브로커에서 메시지가 중복될 경우, 이를 메시지 로그에 기록하지 않고, ack만 전송한다.**

또, sequence + 1 인 메시지가 아닐 경우에도, OutOfOrderSequenceException 을 보내, 메시지 전송 
...
1. 메시지의 key 값이 있는 경우, key 값을 hasing 함. key 값의 hash 를 통해 
