...
# 01-8 Consumer Group 과 Consumer 의 이해
...
### Consumer Group 단위의 독립성
- Kafka는 메시지를 Consumer Group 단위로 관리
- 따라서 서로 다른 Consumer Group은 같은 토픽의 모든 메시지를 독립적으로 소비.

> ex.
> 
> groupA의 Consumer 들은 토픽의 메시지를 각자 분담해서 소비
> 
> groupB의 Consumer 들도 동일한 메시지를 분담해서 소비
>
> 결과적으로, **한 메시지가 groupA와 groupB 모두에게 전달되고, 독립적으로 소비한다.**

- 활용 패턴
  - 멀티 구독 모델:
  - 하나의 이벤트 스트림을 두고, 서로 다른 서비스(다른 group.id 를 가진 consumer groupd으로)가 각자 독립적으로 구독 → 마이크로서비스 아키텍처에서 흔히 사용
