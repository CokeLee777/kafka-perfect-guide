...
![01-1 토픽 개요.png](./images/01-1 토픽 개요.png)
...
docs/soyoung.choi/images/01-2 토픽과파티션.png
...
브로커 b의 토픽만 읽을 수 있다면? 꺼진 브로커의 메시지를 잃는다면? 아주 큰 문제일 것이다.
...
=> relication-factor = 2 
  ...
  producer 에서 send 하면 partition leader에게만 데이터가 간다. 그리고 그 데이터를 
...
01-7-2 Kafka Producer의 send()메소드 호출 프로세스.png
